...
Date: 2026-09-22
Status: designed in chat, approved section by section; awaiting spec review
...
- Stands on the bottom edge near whatever needs attention, with a Clippy-style speech bubble:
  short, lowercase copy, e.g. "it looks like you're building a drop. want me to pick the stems?"
- The bubble carries the current step plus **next · skip · do it for me**. Clicking sssketchy
  opens the full checklist (3 phases, all steps, phase timers). He can be minimised to a corner
  sprite or dismissed (which ends the guided flow, resumable later).
...
- **Tension pass:** at each section boundary, offer the existing automation (filter sweep/swell
  into drops, fade into breakdowns) — toggle + preview each.
...
- Mixing beyond gains, mastering, ear candy generation (risers/impacts) — possible later steps.
