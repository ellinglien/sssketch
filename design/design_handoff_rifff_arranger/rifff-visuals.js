/* Rifff Arranger v1 — drawing helpers, lifted verbatim from the prototype.
...
export function peaksFromChannel(samples, buckets = 128) {
  const out = [];
...
export function downsample(arr, n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    let sum = 0, c = 0;
    for (let j = a; j < Math.max(b, a + 1); j++) { sum += arr[j] || 0; c++; }
    out.push(sum / c);
  }
  return out;
}
...
export function linearWave(peaks) {
  const n = peaks.length, mx = Math.max.apply(null, peaks) || 1;
  let up = "", down = "";
  for (let i = 0; i < n; i++) {
...
export function polarGlyph(peaks, r0 = 17, amp = 20, points = 16) {
  const arr = downsample(peaks, points);
  const n = arr.length, mx = Math.max.apply(null, arr) || 1;
  const pt = (a, r) =>
...
export function offsetLabels(steps, snapDiv, bpm) {
...
export function positionLabel(posInBars) {
