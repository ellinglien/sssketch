 ...
 * band energy, pitch contour -- see the "Analysis caches" section of this
 * project's own CLAUDE.md) for every stem in a project, so the first paint
 * after loading already has real data instead of blank waveforms/radial
 * glyphs that visibly pop in one at a time as each mounted component's own
 * decode happens to finish. Covers every rifff in the project (shelf and
 * placed alike), not just whatever the current arranger mode happens to
 * show right away -- switching modes or scrolling shouldn't re-trigger a
 * visible "still loading" pop-in either.
 *
