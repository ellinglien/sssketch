 ...
 *
 * `ppb` is required, not defaulted — this function used to default it to a
 * fixed constant, and one caller (CollapsedRifffRow) relied on that default
 * by omitting the argument entirely, silently ignoring the real current
 * zoom level (only correct at the default zoom) until it was caught by a
 * real "drop lands in the wrong place" report. Requiring it explicitly
 * means a missing one is a compile error, not a silent wrong answer. */
