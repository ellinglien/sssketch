...
export function ClusterStemsBrowser({
  ...
  // just assigned.
  //
  // This used to be a local recordRoleCategories twin of the wizards' own
  // recordRoleCategorization; both are now the one recordStemRoles
  // (state/stemCategoryCapture.ts), which is what this whole merge is for.
  // Fire-and-forget, matching recordBusCategories exactly; a member whose
  // path doesn't resolve to a real StemCID is silently skipped by the
  // main-process side, not an error here.
  ...
  // still recording the finer category via recordRoles. For the 5 shared
  ...
  function assignDrumSubRole(members: ClusterableStem[], drumSubRole: DrumSubRole): void {
    recordRoles(members, 'drums', drumSubRole)
  }
...
function ClusterRow({
  ...
  // NO free-text note here any more (spec, "The notes field goes"):
  // NOTHING EVER READ IT -- the column, its migration and one INSERT, and
  // not one SELECT anywhere in src/ -- and its own tooltip said it did not
  // affect classifier training. It occupied the busiest control row of the
  // busiest modal in the app, on a surface whose whole value is that the
  // question can be answered in one click, and a text box is the one
  // control that cannot. The SQLite column survives with its rows
  // (riffLibrarySchema.ts); it just stops gaining new ones.
